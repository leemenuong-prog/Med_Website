...
const WORKS = [
  {
    ix: "W·01", t: "Pears — Agent Factory", tag: "AI PRODUCT", year: "2026", dark: false,
  ...
  {
    ix: "W·05", t: "After_Silence", tag: "ARCHITECTURE", year: "2025", dark: true,
    award: "UABB 地外人居板块 · SECTION REPRESENTATIVE — TOP 3%",
  ...
  {
    ix: "W·06", t: "蒋巷文脉 · 科链智谷 2.0", tag: "ARCHITECTURE", year: "2025", dark: true,
    award: "「挑战杯」专项赛 GRAND PRIZE 特等奖 — TOP 1% NATIONWIDE",
    band: [0.6, 1, 0.7, 0.88, 0.52], role: ["Project lead", "挑战杯 2025"],
    body: "Field research found a hollowing-out ancient town sitting one wall away from a hi-tech park — so the project pairs them: a town ↔ industry mutual-empowerment model where heritage supplies identity and the park supplies jobs and flow. An AR heritage-repair workshop (Grasshopper + Fologram) makes restoration tangible and teachable. Grand Prize, top 1% nationwide.",
    zh: "走访中嗅到古镇「空心化」与一墙之隔的高新园区的互补性，构建「产村双向赋能」模式；用 GH + Fologram 搭建可拆解的古建 AR 修复工坊，让修缮过程可见可学。『挑战杯』大学生学术创业科技作品竞赛专项赛特等奖（前 1%），项目组长。",
    metrics: [["TOP 1%", "GRAND PRIZE 特等奖"], ["AR", "GH + FOLOGRAM WORKSHOP"], ["LEAD", "RESEARCH & STRATEGY"]],
  },
  {
    ix: "W·07", t: "环·世界 The Ring-World", tag: "ARCHITECTURE", year: "2024", dark: true,
  ...
  {
    ix: "W·08", t: "上桥 Upper-Via", tag: "ARCHITECTURE", year: "2023", dark: true,
...
function Works({ jump }) {
  ...
  useSecEffect(() => {
    ...
    const works = [...stage.querySelectorAll(".wk-work")];
    ...
    const tick = () => {
      ...
      /* OPEN state — a fanned spread of poster cards */
      ...
      cards.forEach((card, i) => {
        ...
        const cardX = fanCx + off * step - cardW / 2;
        ...
        const cardTop = fanTop + Math.abs(off) * 6 - 64 * e + 16 * rec;
        const ang = (off * angStep) * (1 - 0.92 * e);
        const scl = 1 + 0.2 * e - 0.13 * rec;            /* active ≈+20%, backgrounded ≈-13% */

        /* stretch the bar into the card — same element, continuous geometry */
        const x = lerp(barX, cardX, eGeo);
        const w = lerp(barW, cardW, eGeo);
        const h = lerp(barH, cardH, eGeo);
        const top = lerp(barTop, cardTop, eGeo);
        const rot = ang * eGeo;
        const scale = lerp(1, scl, eGeo);
        ...
        bodies[i].style.opacity = smooth(0.46, 0.92, eS).toFixed(3);
      ...
      /* decoded text below — the active work, crossfaded (kept as designed) */
      works.forEach((wEl, i) => {
        const d = disp - i;
        const op = clp(1 - Math.abs(d) / 0.62, 0, 1) * smooth(0, 0.02, pB) * (1 - close);
        wEl.style.opacity = op.toFixed(3);
        wEl.style.transform = "translateY(" + (-d * 42).toFixed(1) + "px)";
      ...
      <div className="ch-wrap wk-wrap">
        <div className="ch-stage wk-stage" ref={stageRef}>
          ...
          <div className="wk-stack">
            {WORKS.map((wk, i) => (
              <article key={i} className={"wk-work" + (wk.dark ? " ink" : "")} data-hov onClick={() => setOpen(i)}>
                <div className="wk-meta mono"><span className="ix">{wk.ix}</span><span>{wk.tag} · {wk.year}</span></div>
                <h3 className="wk-ttl">{wk.t}<i className="psq" aria-hidden="true"></i></h3>
                <div className="wk-aw mono">{wk.award}</div>
                <div className="wk-metrics">
                  {wk.metrics.map(([b, l], j) => (<div className="m" key={j}><b>{b}</b><span>{l}</span></div>))}
                </div>
                <button className="wk-open mono" data-hov onClick={(ev) => { ev.stopPropagation(); setOpen(i); }}>
                  {wk.link ? "LIVE · OPEN DOSSIER" : "OPEN DOSSIER"}<i className="ar" aria-hidden="true">→</i>
                </button>
              </article>
            ))}
          </div>
